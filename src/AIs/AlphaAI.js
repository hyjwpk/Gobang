...
export class AlphaAI extends AI {
    ...
    async getMove(lastMove) {
        ...
        for (let i = 0; i < this.nPlayout; i++) {
            ...
            await this.playout(boardCopy, lastMove);
        }
    ...
    async playout(state, lastMove) {
        ...
        // 调用策略价值函数进行推理
        const { policy, value } = await this.policyValueFn(state, lastAction);
    ...
    // 调用模型返回策略概率与价值
    async policyValueFn(board, lastAction) {
    ...
    createInputTensor(board, lastAction) {
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                // 当前玩家的棋子
                if (cell === isBlack) squareState[0][row][col] = 1.0;
                // 对手玩家的棋子
                else if (cell !== null) squareState[1][row][col] = 1.0;
            }
        }
    ...
    checkWinState(board, lastRow, lastCol) {
        ...
        for (const [dr, dc] of directions) {
            ...
            if (total >= 5) return { end: true, winner: isBlack };
        }

        const isFull = board.board.flat().every(cell => cell !== null);
        if (isFull) return { end: true, winner: null };
